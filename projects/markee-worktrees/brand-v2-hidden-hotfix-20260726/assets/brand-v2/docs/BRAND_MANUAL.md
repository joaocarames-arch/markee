...
- O `viewBox` final dos wordmarks é `0 0 1481.95 240.00` com padding 20 px acima/abaixo e avanço horizontal respeitando o tracking aplicado.
- Par `k→e` recebe ajuste nativo da fonte (GPOS lookup, -21 em-units) que o script reforça com -25 em-units para compensar a perna diagonal do `k` Sora.
...
| Mobile / favicon 16 px | — | 16 px (validar legibilidade) |
...
- A 16 px a leitura degrada-se (anti-aliasing mais agressivo). Não é recomendado para logótipo — usar apenas favicon.
