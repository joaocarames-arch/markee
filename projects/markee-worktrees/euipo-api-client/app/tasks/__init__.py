...
def build_beat_schedule(policy: SourcePolicy | None = None) -> dict[str, Any]:
    """Build the beat schedule allowed by the source policy.

    The BPI entry is only present when the policy explicitly activates the
    BPI schedule; by default it is absent (BPI containment, default-off).

    Args:
        policy: The policy to apply; defaults to the live settings policy.

    Returns:
        The ``beat_schedule`` mapping for ``celery_app.conf``.
    """
    policy = policy or get_source_policy()
    ...
    if policy.bpi_schedule_active:
        schedule["parse-bpi-daily"] = {
...
def run_async(coro: Coroutine[Any, Any, _T]) -> _T:
    ...
    A dedicated event loop is created and torn down for each invocation. The
    module-level SQLAlchemy async engine keeps pooled asyncpg connections
    bound to the loop that created them, so the engine is disposed inside the
    same loop before it closes; otherwise the next task's fresh loop inherits
    connections attached to a dead loop and fails with
    ...
    """
    from app.core.database import engine  # local import: avoid cycles at import time

    async def _wrapped() -> _T:
        try:
            return await coro
        finally:
            # Release every pooled connection on the loop that owns it.
