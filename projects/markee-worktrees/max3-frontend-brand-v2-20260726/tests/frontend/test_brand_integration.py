...
def test_landing_html_wordmark_src_matches_brand_v2():
    ...
    match = re.search(r'src="(/assets/brand-v2/logos/markee-wordmark-\w+\.svg)"', body)
...
def test_dashboard_renders_accessible_canonical_wordmark_asset():
    ...
    body = H.read_text(H.DASHBOARD_JS)
    asset = "/assets/brand-v2/logos/markee-wordmark-dark.svg"
...
def test_dashboard_wordmark_asset_encodes_lowercase_markee_without_symbol():
    """The rendered asset is six glyph paths, with only the final e in cyan."""
    import xml.etree.ElementTree as ET

    asset = H.BRAND_V2_LOGOS / "markee-wordmark-dark.svg"
    root = ET.fromstring(asset.read_text(encoding="utf-8"))
    namespace = {"svg": "http://www.w3.org/2000/svg"}
    paths = root.findall(".//svg:g/svg:path", namespace)
    fills = [path.attrib.get("fill", "").upper() for path in paths]

    assert len(paths) == 6, "lowercase markee must contain m, a, r, k, e, e only"
    assert fills[:5] == ["#E8E8E8"] * 5
    assert fills[-1] == "#35D0E0", "only the last e may carry the cyan accent"
