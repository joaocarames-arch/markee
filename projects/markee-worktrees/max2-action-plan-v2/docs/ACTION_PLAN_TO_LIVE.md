...
- **Confirmado:** master contém HEAD `51aa7d0`; o corpus de planeamento em `docs/ACTION_PLAN_TO_LIVE.md`, `docs/execution/`, `docs/research/`, `contents/` e o YAML BPI é untracked na árvore master de origem.
- **Confirmado:** WP1 tem branch/worktree `stg00-bpi-containment` em `0a5e928`.
- **Confirmado:** review reporta o patch de segurança aplicável a `51aa7d0` e 152 testes/4 skips após correcções no snapshot; isto não prova que o patch foi aplicado neste worktree.
- **Confirmado:** WP2 está em progresso noutro local/worktree; branch, commit final e evidence report não estão disponíveis nesta missão.
- **UNKNOWN:** merge de WP1, aplicação do patch de WP2, suite actual, runtime, BD, Docker, staging, produção e gates. Não são promovidos por inferência.

## 6. Fases e work packages

### F0 — Baseline imediato

**WP2 — Security baseline.** Aplicar o patch de segurança sobre master e integrar a contenção WP1, resolvendo apenas o conflito conhecido em `app/tasks/__init__.py`: conservar o beat schedule condicionado do WP1 e `run_async` com `engine.dispose()` do patch. Validar suite completa, `tests/stg00` e `tests/integration/test_security_fixes.py`. Evidence: `evidence/stg00/wp2-security-baseline.md`. Merge em master fecha F0, sujeito a auditoria. Estado overnight: `in_progress` noutro worktree; não validado aqui.
