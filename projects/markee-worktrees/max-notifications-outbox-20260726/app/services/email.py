...
class EmailBackend(Protocol):
    ...
    async def send(self, envelope: EmailEnvelope) -> str:
        """Dispatch an envelope and return a provider-safe message identifier."""
...
class InMemoryEmailGateway:
    ...
    async def send(self, envelope: EmailEnvelope) -> str:
        ...
        message_id = hashlib.sha256(
            f"{len(self._sent)}:{envelope.headers.get('X-Markee-Dedupe-Key', '')}".encode()
        ).hexdigest()[:24]
        logger.info("in-memory email accepted message_id=%s", message_id)
        return f"memory-{message_id}"
...
class SMTPEmailGateway:
    ...
    async def send(self, envelope: EmailEnvelope) -> str:
        ...
        try:
            ...
            raw_message_id = str(message.get("Message-ID") or "")
            if not raw_message_id:
                raw_message_id = hashlib.sha256(message.as_bytes()).hexdigest()[:24]
            return f"smtp-{hashlib.sha256(raw_message_id.encode()).hexdigest()[:24]}"
