...
from app.services.notifications import AlertNotificationEvent, NotificationOutboxService
...
class AlertService:
    ...
    async def is_trademark_source_denied(self, trademark_id: str | uuid.UUID | None) -> bool:
        ...
        source_name = (
            await self.db.execute(
                ...
                .where(Trademark.id == uuid.UUID(str(trademark_id)))
            )
        ).scalar_one_or_none()
        return get_source_policy().is_source_denied(source_name)

    async def _enqueue_alert_notification(
        self,
        alert: Alert,
        *,
        ...
        """Queue an owner-only email intent in the caller's transaction."""
        user = await self.db.get(User, alert.user_id)
        if user is None or not user.email:
            raise ValueError("alert owner has no email recipient")
        ...
        return await NotificationOutboxService(self.db).enqueue_event(event)
    ...
    async def generate_similarity_alert(
        ...
        """
        if await self.is_trademark_source_denied(trademark_id):
            logger.warning(
                "Similarity alert suppressed for denied-source trademark %s",
                trademark_id,
            )
            return None
        trademark = await self.db.get(Trademark, trademark_id)
        item = (
            await self.db.get(WatchlistItem, watchlist_item_id)
        ...
        self.db.add(alert)
        await self.db.flush()
        await self._enqueue_alert_notification(alert)
        return alert
    ...
    async def generate_deadline_alert(
        ...
        self.db.add(alert)
        await self.db.flush()
        await self._enqueue_alert_notification(
            alert,
            due_date=due_date,
            days_remaining=days_remaining,
        )
        ...
        result = await self.db.execute(
            select(Alert).where(
                and_(
                    ...
                    Alert.trademark_id == trademark_id,
