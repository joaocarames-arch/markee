...
const ROUTES = ['/dashboard', '/search', '/watchlists', '/alerts', '/deadlines', '/settings', '/login'];
...
async function request(method, path, opts) {
  ...
  // Handle auth expiry globally. The thrown error is flagged so views
  // skip their generic retry UI while the router redirects to login.
  if (response.status === 401) {
    clearToken();
    if (currentPath() !== '/login') navigate('/login');
    const authError = new Error('Sessão expirada. Inicie sessão novamente.');
    authError.authRedirect = true;
    throw authError;
  }
...
/**
 * Render the 404 state for an unknown hash route inside the shell view.
 * @param {string} path - The requested (unknown) hash path.
 */
function renderNotFound(path) {
  ...
  setTopbarActions('');
  view.innerHTML = `
    <div class="glass-card shell-state shell-state-not-found" role="alert" aria-live="polite">
      <div class="state-icon">${icon('search', 40)}</div>
      <h2>Página não encontrada</h2>
      <p>O caminho <code class="shell-state-path">${esc(path)}</code> não existe.</p>
      <button class="btn btn-primary" id="notfound-cta" type="button">Voltar ao painel</button>
    </div>`;
  ...
  // Escape also returns to the dashboard; unbind on the next navigation
  // so the listener cannot leak into other views.
...
/**
 * Render a reusable API failure state inside a content container.
 * @param {string} message - PT-PT description of the failure.
 * @param {string} retryId - DOM id for the retry button.
 * @param {function} onRetry - Callback that re-runs the failed request.
 * @param {HTMLElement} [container] - Target element; defaults to the view.
 */
function renderApiError(message, retryId, onRetry, container) {
  const target = container || getView();
  if (!target) return;
  target.innerHTML = `
    <div class="shell-state shell-state-error" role="alert">
      <div class="state-icon">${icon('close', 40)}</div>
      <h3>Algo correu mal</h3>
      <p>${esc(message || 'Não foi possível carregar os dados.')}</p>
      <p class="visually-hidden" id="${esc(retryId)}-status" role="status" aria-live="polite"></p>
      <button class="btn btn-primary" id="${esc(retryId)}" type="button"
              aria-label="Tentar novamente carregar os dados">
        ${icon('refresh', 16)} Tentar novamente
      </button>
    </div>`;

  const btn = document.getElementById(retryId);
  if (!btn || typeof onRetry !== 'function') return;
  const status = document.getElementById(`${retryId}-status`);
  btn.addEventListener('click', async () => {
    if (btn.disabled) return;
    btn.disabled = true;
    if (status) status.textContent = 'A tentar novamente…';
    try {
      await onRetry();
    } catch (_) {
      /* the retried renderer owns its own failure state */
    } finally {
      // Retries normally re-render this container; only restore when the
      // original button is still mounted (i.e. the retry failed fast).
    ...
    <div class="shell">
      <aside class="sidebar${state.sidebarOpen ? ' open' : ''}" id="sidebar" aria-label="Navegação principal">
        <div class="sidebar-brand">
          <img class="dashboard-wordmark" src="/assets/brand-v2/logos/markee-wordmark-dark.svg?v=brand-v2-live-hotfix-20260726" alt="markee" />
        </div>
  ...
  root.innerHTML = `
    <div class="auth-wrap">
      <div class="glass-card auth-card">
        <div class="auth-brand"><img class="dashboard-wordmark" src="/assets/brand-v2/logos/markee-wordmark-dark.svg?v=brand-v2-live-hotfix-20260726" alt="markee" /></div>
...
async function renderDashboard() {
  ...
  try {
    ...
    renderApiError(err.message, 'retry-dashboard', renderDashboard);
  }
}
...
async function router() {
  ...
  // Reject near-misses outright: only exact known routes render a view.
  // Anything else gets the 404 state inside the shell — never a silent
  ...
  const renderer = VIEW_RENDERERS[path];
  try {
    await renderer();
  } catch (err) {
    const v = getView();
