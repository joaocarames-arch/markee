...
async def list_trademarks(
    ...
    if not rows and q:
        # Fall back to the EUIPO (mock) service so the UI is never empty in dev.
        service = get_euipo_service()
        return [TrademarkOut(**record) for record in service._mock_search(q, limit)]
