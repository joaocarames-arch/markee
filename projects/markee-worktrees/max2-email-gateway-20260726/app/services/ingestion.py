...
class BPIIngestSummary:
    ...
    unmatched: int = 0
    # Events dropped because BPI ingestion is disabled (kill switch).
    ...
    async def ingest_bpi_events(
        ...
        *,
        ...
        if not get_source_policy().bpi_ingestion_active:
            # BPI containment: while the kill switch is off no lifecycle
            # events, review-queue items or any other rows may be written.
            summary.skipped_disabled = len(events)
            logger.warning(
                "BPI ingestion disabled; dropped %d event(s)", len(events)
            )
            return summary
        for event in events:
