...
class Settings(BaseSettings):
    ...
    # ── Email (transactional gateway) ───────────────────────────────────────
    # ``memory`` keeps every envelope in process (tests + dev). ``smtp`` opens
    # an aiosmtplib connection to the configured server. Any other value is
    ...
    SMTP_TIMEOUT: int = 10
    # Allowlist of bare addresses (``"Name <addr@host>"`` also supported). The
    # SMTP/NULL gateway refuses to send when the chosen sender is not on this
    # list. Outside development/test, a populated allowlist is mandatory.
    ALLOWED_FROM_ADDRESSES: list[str] = ["no-reply@markee.pt"]
    # Verification tokens: TTL in minutes and a per-user rolling rate limit.
    EMAIL_VERIFY_TTL_MINUTES: int = 60 * 24
    EMAIL_VERIFY_RATE_LIMIT_PER_HOUR: int = 5
    # Public base URL we mint verification links under. Must be in the
    # allowlist below — anything else is rejected.
    EMAIL_VERIFICATION_URL_BASE: str = "http://localhost:8000"
    VERIFICATION_URL_ALLOWED_BASES: list[str] = [
        "http://localhost:8000",
        "http://127.0.0.1:8000",
        "https://markee.pt",
    ]
...
def _validate(settings: Settings) -> Settings:
    ...
    if settings.ENVIRONMENT != "development":
        ...
        if email_provider != "smtp":
            problems.append("EMAIL_PROVIDER must be smtp outside development")
        ...
        if not settings.ALLOWED_FROM_ADDRESSES:
            problems.append("ALLOWED_FROM_ADDRESSES must be non-empty")
        if email_provider == "smtp" and not (
            settings.SMTP_HOST and settings.SMTP_USER and settings.SMTP_PASSWORD
        ):
            problems.append("SMTP credentials (HOST/USER/PASSWORD) required")
        if email_provider == "smtp" and not settings.SMTP_USE_TLS:
            problems.append("SMTP_USE_TLS must be True outside development")
        if settings.SMTP_TIMEOUT <= 0:
            problems.append("SMTP_TIMEOUT must be positive")
        if (
            settings.EMAIL_VERIFICATION_URL_BASE
            not in settings.VERIFICATION_URL_ALLOWED_BASES
        ):
            problems.append(
                "EMAIL_VERIFICATION_URL_BASE must be in VERIFICATION_URL_ALLOWED_BASES"
            )
        if problems:
            raise RuntimeError(
