...
async def get_current_user(
    ...
    if await is_token_blocklisted(token):
        # Token was invalidated by logout before its natural expiry.
        raise credentials_exception
...
async def login(
    ...
    Failed attempts are rate limited per identifier+IP; when Redis is
    unavailable the limiter fails open and the attempt proceeds.

    Args:
        request: The incoming request (source of the client IP).
    ...
    Raises:
        HTTPException: 401 if the credentials are invalid, 429 when the
            failed-attempt threshold for this identifier+IP is exceeded.
    """
    client_ip = request.client.host if request.client else None
    rate_key = login_rate_limit_key(form_data.username, client_ip)
    retry_after = await get_login_retry_after(rate_key)
    if retry_after is not None:
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="Demasiadas tentativas de início de sessão. Tente novamente mais tarde.",
    ...
    if not user or not verify_password(form_data.password, user.hashed_password):
        await register_failed_login(rate_key)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    await reset_login_attempts(rate_key)
    return Token(access_token=token, token_type="bearer")


@router.post("/logout", status_code=status.HTTP_204_NO_CONTENT)
async def logout(token: str = Depends(oauth2_scheme)) -> Response:
    """Invalidate the presented bearer token server-side (idempotent).

    Args:
        token: The JWT bearer token to invalidate.

    Returns:
        An empty 204 response, including for repeated logouts with the same
        token.
