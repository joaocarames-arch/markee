...
class TestAuthEndpoints:
    ...
    def test_login_success(self, mock_db_session):
        ...
        # bcrypt hash de "secret123" (via app.core.security, sem passlib)
...
def _authenticated_headers(result_mock) -> dict[str, str]:
    """Configura o result_mock para resolver um utilizador ativo e devolve
    os headers Bearer correspondentes (endpoints de marcas exigem auth)."""
    user_mock = MagicMock()
    user_mock.id = uuid4()
    user_mock.is_active = True
    result_mock.scalar_one_or_none.return_value = user_mock
    token = create_access_token({"sub": str(user_mock.id)})
    return {"Authorization": f"Bearer {token}"}


class TestTrademarkEndpoints:
    """Trademark search com BD mockado e fallback EUIPO (requer auth)."""

    def test_list_trademarks_empty_db_no_fallback_by_default(self, mock_db_session):
        # Simular BD vazia — sem ENABLE_MOCK_FALLBACK a resposta é vazia
        result_mock = MagicMock()
        result_mock.scalars.return_value.all.return_value = []
        mock_db_session.execute.return_value = result_mock
        headers = _authenticated_headers(result_mock)

        response = client.get("/api/v1/trademarks?q=Markee", headers=headers)
        assert response.status_code == 200
        assert response.json() == []

    def test_list_trademarks_empty_db_with_fallback(self, mock_db_session, monkeypatch):
        # Com o fallback explicitamente ativo, os registos vêm marcados MOCK/
        ...
        assert data[0]["status"].startswith("MOCK/")
    ...
    def test_list_trademarks_no_query(self, mock_db_session):
        ...
        response = client.get("/api/v1/trademarks", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
    ...
    def test_get_trademark_by_number_found(self, mock_db_session):
        ...
        result_mock = MagicMock()
        headers = _authenticated_headers(result_mock)
        # 1ª chamada resolve o utilizador autenticado, 2ª a marca.
        user_mock = result_mock.scalar_one_or_none.return_value
        result_mock.scalar_one_or_none.side_effect = [user_mock, tm_mock]
        mock_db_session.execute.return_value = result_mock

        response = client.get("/api/v1/trademarks/N.123456", headers=headers)
        assert response.status_code == 200
    ...
    def test_get_trademark_by_number_not_found(self, mock_db_session):
        ...
        headers = _authenticated_headers(result_mock)
        user_mock = result_mock.scalar_one_or_none.return_value
        result_mock.scalar_one_or_none.side_effect = [user_mock, None]
        mock_db_session.execute.return_value = result_mock

        response = client.get("/api/v1/trademarks/N.999999", headers=headers)
